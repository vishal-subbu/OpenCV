...
void thresh_callback(int, void* )
{
  ...
  Mat last = Mat::zeros( canny_output.size(), CV_8UC3 );
 ...
 if(diff<0)
   diff=(-1*diff);
     ...
     Point beg;Point mid;
    last= drawing;
    circle(last, centre, 3, Scalar(0, 255, 0), CV_FILLED, CV_AA);
        circle(last, top.pt, 3, Scalar(255,0,0), CV_FILLED, CV_AA);
        circle(last, side.pt, 3, Scalar(0, 255,0), CV_FILLED, CV_AA);
    cout<<top.pt.x<<"  " <<top.pt.y<<endl;
          ...
          while(pointPolygonTest(poly_final,down1,false)!=-1) 
            {
             ...
             y+=tan; 
             down1.y=y;
             circle(last, down1, 3, Scalar(255, 255,255), CV_FILLED, CV_AA);
           imshow("Last",last);
           cout<<pointPolygonTest(poly_final,down1,false)<<endl;
          waitKey(0);
            }
...
{ 
       ...
       while(pointPolygonTest(poly_final,mid,true)>2 || pointPolygonTest(poly_final,mid,true)<-2)
            ...
            { cout<<" dy +ve"<<endl;
              ...
              while(pointPolygonTest(poly_final,down1,false)!=-1) 
               {cout<<"Entered while loop"<<endl;
                down1.y++; circle(last, down1, 3, Scalar(255, 255,255), CV_FILLED, CV_AA);
          imshow("Last",last);
          cout<<pointPolygonTest(poly_final,down1,false)<<endl;
          waitKey(0);
               }
        ...
        circle(drawing, down.pt, 3, Scalar(0, 255,0), CV_FILLED, CV_AA);
